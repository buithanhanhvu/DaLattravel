# 🧪 KẾ HOẠCH & MA TRẬN KIỂM THỬ TỔNG THỂ DỰ ÁN DALATTRAVEL
...
# 📄 PHẦN 1: KẾ HOẠCH KIỂM THỬ TỔNG THỂ (MASTER TEST PLAN)

## 1. GIỚI THIỆU & MỤC TIÊU (INTRODUCTION & OBJECTIVES)
Tài liệu Kế hoạch Kiểm thử này xác định chiến lược, phạm vi, môi trường, nguồn lực và lịch trình kiểm thử cho toàn bộ hệ thống du lịch thông minh **DaLatTravel** (Backend Spring Boot 4.1.0 + MySQL 8.0 + Frontend Thymeleaf/Bootstrap 5 + OpenStreetMap OSRM + Google OAuth2).

### 🎯 Mục tiêu chất lượng (Quality Goals):
- **Tính đúng đắn về chức năng (Functional Correctness):** Đảm bảo 100% các luồng nghiệp vụ Lập lịch trình tự động bằng thuật toán TSP, Ghép xe đi chung, Đặt phòng khách sạn và Quản trị Admin chính xác theo yêu cầu.
- **Tính toàn vẹn dữ liệu & Tính giá (Data Integrity & Calculation):** Đảm bảo không bị sai lệch tổng chi phí đặt phòng theo số đêm thực tế, tính toán mã đơn `DLBK-XXXX` duy nhất và lưu vết CSDL an toàn.
- **Tính an toàn & bảo mật (Security & RBAC):** Kiểm tra cơ chế phân quyền (`AuthInterceptor`), bảo mật tài nguyên `/admin`, băm mật khẩu SHA-256 và xác thực an toàn với Google Identity OAuth2.
- **Độ tin cậy của thuật toán & API (Algorithm & API Reliability):** Đảm bảo thuật toán TSP sắp xếp lịch trình tối ưu và API OSRM trả về thông tin định tuyến (km, phút) chuẩn xác.
...
- **Xác thực người dùng (Auth & Security):** Đăng ký tài khoản, Đăng nhập thường & Google Identity OAuth2 (JWT Decode), Mã hóa mật khẩu SHA-256, Chặn `AuthInterceptor` bảo mật tài nguyên `/admin/**`, Đăng xuất giải phóng Session.
- **Lên lịch trình tự động (Trip Planner & OSRM):** Lập lịch trình 1-3 ngày tự động bằng thuật toán TSP, Định tuyến OSRM thực tế (km/phút), Định vị GPS hiện tại, Hiển thị đường đi Polyline trên bản đồ LeafletJS.
- **Ghép xe đi chung (Carpooling):** Tìm kiếm chuyến xe ghép theo lộ trình (TP.HCM, Sân bay Liên Khương, Đà Lạt), Bộ lọc điểm đi/đến, Đăng tin tìm xe / ghép chuyến.
...
- **Quản trị viên (Admin Dashboard & CRUD):** Dashboard thống kê chỉ số thời gian thực, Duyệt/Hủy đơn đặt phòng (`PENDING` $\rightarrow$ `CONFIRMED`/`CANCELLED`), CRUD Khách sạn, Địa điểm du lịch, Nhà hàng, Quản lý phân quyền tài khoản (RBAC `USER` $\leftrightarrow$ `ADMIN`).
...
1. **Kiểm thử Hộp trắng (White-box Testing / Unit Testing):**
   - **Công cụ:** JUnit 5, Mockito (`@Mock`, `@InjectMocks`, `when().thenReturn()`).
   - **Mã nguồn test:** `AuthServiceTest.java` (Test logic đăng ký, đăng nhập, băm mật khẩu), `HotelBookingServiceTest.java` (Test logic tính tiền đặt phòng và xử lý biên ngày checkin/checkout).
   - **Kết quả:** `6/6 Unit Tests PASSED (100% Pass Rate)`.

2. **Kiểm thử Hộp xám (Gray-box Testing / API & Security Testing):**
   - **Công cụ:** Postman, Spring Boot MockMvc, `AuthInterceptor`.
   - **Nội dung:** Kiểm tra Response HTTP Status Code (200 OK, 400 Bad Request, 401 Unauthorized, 403 Forbidden), cấu trúc Body JSON và chặn truy cập bất hợp pháp vào tuyến đường `/admin`.

3. **Kiểm thử Hộp đen (Black-box Testing / System & UI E2E Testing):**
   - **Công cụ:** Browser Subagent (Playwright/Selenium), Manual Testing.
   - **Kỹ thuật thiết kế Test Case:**
     - **Phân vùng tương đương (Equivalence Partitioning):** Phân chia dữ liệu đầu vào hợp lệ và không hợp lệ (Trùng username/email, mật khẩu yếu, chọn ngày trả phòng nhỏ hơn ngày nhận phòng).
     - **Phân tích giá trị biên (Boundary Value Analysis):** Kiểm thử tại các mốc biên (Số đêm lưu trú = 1, Số khách = 1 đến 10, Mật khẩu = 6 ký tự).

---

## 4. MÔI TRƯỜNG KIỂM THỬ (TEST ENVIRONMENT)
- **Backend Runtime:** Java OpenJDK 21, Spring Boot 4.1.0.
- **Database:** MySQL Server 8.0 (Database: `dalattravel_db`, Port: 3306).
- **Frontend Runtime:** HTML5, Vanilla CSS, Bootstrap 5.3, Thymeleaf 3.1.
- **Bản đồ & Routing API:** OpenStreetMap OSRM REST Routing API, LeafletJS v1.9.4.
- **Trình duyệt kiểm thử:** Google Chrome Version 120+.

---

## 5. TIÊU CHÍ DỪNG & CHUYỂN GIAO (ENTRY / EXIT CRITERIA)
- **Tiêu chí bắt đầu (Entry Criteria):** Mã nguồn Backend biên dịch thành công (`BUILD SUCCESS`), MySQL `dalattravel_db` nạp sẵn 50 địa điểm, 10 khách sạn, 12 nhà hàng.
- **Tiêu chí hoàn thành (Exit Criteria):** 100% các test case trong JUnit/Mockito vượt qua (`6/6 Passed`), 100% kịch bản kiểm thử trong ma trận đạt **PASSED**, không còn bug Critical/High.

---

## 6. QUẢN LÝ LỖI (DEFECT MANAGEMENT PROCESS)
Các lỗi phát hiện trong quá trình kiểm thử được ghi nhận vào nhật ký lỗi **`BUG_REPORT.md`** theo quy chuẩn MantisBT / Jira bao gồm các trường:
...
- **Priority (Độ ưu tiên xử lý):** P1 (Khẩn cấp) $\rightarrow$ P4 (Thấp).
- **Steps to Reproduce:** Các bước chi tiết để tái hiện lỗi.
- **Expected Result vs Actual Result:** Đối chiếu kết quả mong đợi với kết quả thực tế.
- **Fix Solution:** Giải pháp đã khắc phục trong mã nguồn.

---

# 📊 PHẦN 2: MA TRẬN KỊCH BẢN KIỂM THỬ CHI TIẾT (TEST CASES MATRIX)

## 👨‍💻 PHẦN 2.1: PHÂN HỆ XÁC THỰC & NGƯỜI DÙNG (AUTHENTICATION & USER)
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **TC_AUTH_001** | Đăng ký | Đăng ký tài khoản người dùng thành công | Form đăng ký mở, Username chưa tồn tại | 1. Mở trang `/register`<br>2. Nhập thông tin hợp lệ<br>3. Bấm "Hoàn tất đăng ký" | Username: `qa_user_01`<br>Email: `qa01@gmail.com`<br>Pass: `123456`<br>FullName: `QA User 01`<br>Phone: `0901234567` | Đăng ký thành công, thông báo thành công, chuyển hướng về `/login`. Mật khẩu băm SHA-256. | Hộp đen (Normal Flow) |
...
| **TC_AUTH_005** | Đăng nhập | Đăng nhập tài khoản ADMIN thành công | Tài khoản `admin` / `admin123` có `role = "ADMIN"` | 1. Mở trang `/login`<br>2. Điền username `admin` & pass `admin123`<br>3. Bấm "Đăng nhập" | User: `admin`<br>Pass: `admin123` | Đăng nhập thành công, Redirect trực tiếp tới `/admin`. Navbar hiển thị nút badge "Admin" vàng. | Normal Flow (RBAC) |
...
| **TC_AUTH_007** | Đăng nhập Google | Đăng nhập bằng Google Identity Services (OAuth2) | Client ID Google hợp lệ | 1. Mở trang `/login`<br>2. Nhấp nút "Đăng nhập bằng Google"<br>3. Google trả vể JWT Credential Token | Client ID: `1071806914161-...`<br>Credential: `eyJhbGci...` | Gửi POST tới `/login/google`, decode JWT thành công, tự tạo/liên kết tài khoản Google & lập Session. | OAuth2 JWT Flow |
...
## 🗺️ PHẦN 2.2: PHÂN HỆ LÊN LỊCH TRÌNH TỰ ĐỘNG & BẢN ĐỒ (TRIP PLANNER, TSP & OSRM)
...
## 🚗 PHẦN 2.3: PHÂN HỆ GHÉP XE ĐI CHUNG & ĐẶT PHÒNG KHÁCH SẠN (CARPOOL & HOTEL BOOKING)
...
## 👑 PHẦN 2.4: PHÂN HỆ QUẢN TRỊ VIÊN & PHÂN QUYỀN (ADMIN & RBAC)
...
## 💻 PHẦN 2.5: CÁC KỊCH BẢN KIỂM THỬ GIAO DIỆN & TRẢI NGHIỆM (UI/UX AUTOMATION TEST CASES)
...
## 🛡️ PHẦN 2.6: CÁC KỊCH BẢN KIỂM THỬ API, PHÂN QUYỀN & BẢO MẬT (API AUTOMATION & SECURITY TEST CASES)
...
# 🐛 PHẦN 3: NHẬT KÝ THEO DÕI & QUẢN LÝ LỖI (DEFECT LOG / BUG REPORT)
...
## 📋 BÁO CÁO CÁC LỖI NỔI BẬT ĐÃ PHÁT HIỆN & KHẮC PHỤC (DEFECT REPORTS)

### 🔴 BUG-001: Người dùng không có quyền ADMIN vẫn truy cập được trang Quản trị khi gõ URL trực tiếp

- **Bug ID:** `BUG-001`
- **Tiêu đề:** Khách vỡ lòng hoặc tài khoản role `USER` nhập URL `/admin` trên trình duyệt vẫn xem được nội dung Quản trị.
...
- **Kết quả mong đợi (Expected Result):** Từ chối truy cập, chuyển hướng về `/login?error=forbidden` kèm thông báo lỗi.
- **Giải pháp khắc phục (Fix Solution):** Xây dựng `AuthInterceptor.java` triển khai `HandlerInterceptor`, kiểm tra `session.loggedInUser` và `user.getRole().equalsIgnoreCase("ADMIN")`. Đăng ký interceptor trong `WebMvcConfig.java` cho tuyến đường `/admin/**`.

---

### 🟠 BUG-002: Sai lệch tổng chi phí đặt phòng khi chọn ngày Check-out nhỏ hơn Check-in

- **Bug ID:** `BUG-002`
- **Tiêu đề:** Tổng tiền đặt phòng bị tính âm hoặc bằng 0 VNĐ khi chọn ngày trả phòng trước ngày nhận phòng.
...
- **Kết quả thực tế (trước khi sửa):** Tổng tiền lưu vào CSDL bị tính âm hoặc bằng 0 VNĐ.
- **Kết quả mong đợi:** Hệ thống tự động tính tối thiểu 1 đêm lưu trú, tổng tiền luôn chính xác >= giá 1 đêm.
- **Giải pháp khắc phục:** Bổ sung logic kiểm tra `ChronoUnit.DAYS.between(checkIn, checkOut)` trong `HotelController.java`. Nếu `days <= 0`, tự động điều chỉnh `days = 1`.
